...
public class RatingStudentsService {
	...
	{
		HashSet<String> sub=(HashSet<String>) arr.findBySubject(ab.getSubject());
		ArrayList<String> subjectName=new ArrayList<>(sub);
		String data="";
		for(int k=0;k<subjectName.size();k++)
		{
		List<String> list1=arr.findByStudentName1(ab.getStudentName(),subjectName.get(k));
		List<Integer> list2=arr.findByStudentName2(ab.getStudentName(),subjectName.get(k));
	...
	{
		HashSet<String> sub=(HashSet<String>) arr.findByName(ab.getStudentName());
		ArrayList<String> stuName=new ArrayList<>(sub);
		String data="";
		for(int k=0;k<stuName.size();k++)
		{
		List<String> list1=arr.findByStudentName1(stuName.get(k),ab.getSubject());
		List<Integer> list2=arr.findByStudentName2(stuName.get(k),ab.getSubject());
		LinkedHashMap<String, Integer> map1=new LinkedHashMap<String, Integer>();
		if(list1.size()>0&&list2.size()>0) 
		{	
			for(int i=0;i<list1.size();i++)
